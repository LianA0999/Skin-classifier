...
.navbar { background-color: #ffffff; border-bottom: 1px solid #E0EDD8; padding: 0 4rem; display: flex; align-items: center; justify-content: space-between; height: 64px; position: sticky; top: 0; z-index: 999; box-shadow: 0 1px 8px rgba(0,0,0,0.06); }
.navbar-logo { font-size: 1.3rem; font-weight: 800; color: #2E6B35; display: flex; align-items: center; gap: 0.5rem; }
.navbar-links { display: flex; gap: 2rem; align-items: center; }
.navbar-link { color: #555; font-size: 0.92rem; font-weight: 600; text-decoration: none; cursor: pointer; padding: 0.3rem 0; }
.navbar-cta { background-color: #4CAF50; color: white; padding: 0.5rem 1.3rem; border-radius: 8px; font-weight: 700; font-size: 0.88rem; cursor: pointer; }
...
.hero-sub { font-size: 1.05rem; color: #666; max-width: 560px; margin: 0 auto 2rem; line-height: 1.7; }
...
.feature-icon { font-size: 1.8rem; margin-top: 2px; }
...
st.markdown("""
<div class="navbar">
    <div class="navbar-logo">🌿 DermaVision</div>
    <div class="navbar-links">
        <span class="navbar-link">Home</span>
        <span class="navbar-link">Skin Analysis</span>
        <span class="navbar-link">About Us</span>
    </div>
    <span class="navbar-cta">Start Analysis</span>
</div>
...
if st.session_state.page == "home":
    st.markdown("""
    <div class="hero">
        <div class="hero-badge">🌿 Free AI Skin Analysis</div>
    ...
    st.markdown("<h1 style='font-size:5rem; text-align:center; color:#1B5E20; font-weight:900;'>DermaVision</h1>", unsafe_allow_html=True)
    ...
    st.markdown("""
    <div class='feature-item'><div class='feature-icon'>👤</div><div class='feature-text'><h4>Tell us about yourself</h4><p>Enter your age group and sensitivity so we can personalise your results.</p></div></div>
    <div class='feature-item'><div class='feature-icon'>📸</div><div class='feature-text'><h4>Take or upload a photo</h4><p>Use your camera for a live capture, or upload an existing front-facing photo.</p></div></div>
    <div class='feature-item'><div class='feature-icon'>🤖</div><div class='feature-text'><h4>AI detects your skin condition</h4><p>Our EfficientNetB3 model analyses your image for 7 common skin conditions and returns personalised ingredient recommendations.</p></div></div>
    """, unsafe_allow_html=True)
    st.markdown("<div class='divider'></div>", unsafe_allow_html=True)
    ...
    c1, c2 = st.columns(2)
    for i, (icon, name) in enumerate([("🔴","Acne"),("💧","Oiliness"),("🌵","Dryness"),("🟤","Dark Spots"),("🌹","Redness"),("〰️","Wrinkles"),("✅","Normal Skin")]):
        with c1 if i % 2 == 0 else c2:
            st.markdown(f"<div style='background:#fff;border:1px solid #E0EDD8;border-radius:10px;padding:0.8rem 1rem;margin-bottom:0.7rem;display:flex;align-items:center;gap:0.7rem;'><span style='font-size:1.3rem;'>{icon}</span><span style='font-weight:600;color:#3D3D3D;font-size:0.92rem;'>{name}</span></div>", unsafe_allow_html=True)
    st.markdown("<div class='divider'></div>", unsafe_allow_html=True)
    st.markdown("<div class='disclaimer'><p>⚠️ <strong>Disclaimer:</strong> DermaVision is an educational tool and is not a substitute for professional medical advice. Always consult a qualified dermatologist for clinical diagnosis and personalised treatment plans.</p></div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if st.button("← Back to Home", key="back_home"):
        st.session_state.page = "home"; st.session_state.img = None; st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("<div class='input-card'><div class='input-card-icon'>📷</div><div class='input-card-title'>Live Camera</div><div class='input-card-sub'>Take a photo using your device camera</div></div>", unsafe_allow_html=True)
        if st.button("Use Camera", key="cam_btn"):
            st.session_state.input_method = "camera"; st.session_state.page = "capture"; st.rerun()
    with c2:
        st.markdown("<div class='input-card'><div class='input-card-icon'>🖼️</div><div class='input-card-title'>Upload Photo</div><div class='input-card-sub'>Select an existing image from your device</div></div>", unsafe_allow_html=True)
        if st.button("Upload Photo", key="upload_btn"):
            st.session_state.input_method = "upload"; st.session_state.page = "capture"; st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if st.session_state.input_method == "camera":
        ...
        if shot:
            ...
            if is_blurry(img):
                ...
                with bc1:
                    if st.button("🔄 Retake Photo", key="retake"): st.rerun()
                with bc2:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if st.button("← Back", key="back_result"):
        st.session_state.page = "capture"; st.session_state.img = None; st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("<div class='section-heading'>Your Results</div>", unsafe_allow_html=True)
    st.markdown(f"<div style='background:#F1F8F1;border:1px solid #C8E6C9;border-radius:10px;padding:0.8rem 1.2rem;margin-bottom:1.5rem;font-size:0.88rem;color:#2E6B35;'>👤 <strong>Age:</strong> {st.session_state.age_group} &nbsp;|&nbsp; <strong>Sensitive:</strong> {st.session_state.sensitivity}</div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with c1:
        st.markdown("<div class='team-card'><div class='team-avatar'>👤</div><div class='team-name'>L</div><div class='team-role'>AI Engineer</div><div class='team-bio'>Specialises in deep learning and computer vision. Led the model training and dataset pipeline for DermaVision.</div><div class='team-contact'>📧 l@email.com</div></div>", unsafe_allow_html=True)
    with c2:
        st.markdown("<div class='team-card'><div class='team-avatar'>👤</div><div class='team-name'>A</div><div class='team-role'>Data Scientist</div><div class='team-bio'>Focused on data collection, cleaning, and analysis. Curated the skin condition datasets used to train the model.</div><div class='team-contact'>📧 a@email.com</div></div>", unsafe_allow_html=True)
    with c3:
        st.markdown("<div class='team-card'><div class='team-avatar'>👤</div><div class='team-name'>M</div><div class='team-role'>Frontend Developer</div><div class='team-bio'>Designed and built the DermaVision interface. Focused on creating a clean, accessible user experience.</div><div class='team-contact'>📧 m@email.com</div></div>", unsafe_allow_html=True)
    st.markdown("<div class='divider'></div>", unsafe_allow_html=True)
    st.markdown("""
